(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.GodwokenStoreCore = {}));
  ...
  class NumberHash {
    ...
    validate(compatible = false) {
      assertDataLength(this.view.byteLength, NumberHash.size());
  ...
  function SerializeNumberHash(value) {
  ...
  class L2BlockCommittedInfo {
    ...
    getTransactionHash() {
      return new Byte32(this.view.buffer.slice(0 + Uint64.size() + Byte32.size(), 0 + Uint64.size() + Byte32.size() + Byte32.size()), { validate: false });
    }

    validate(compatible = false) {
      assertDataLength(this.view.byteLength, L2BlockCommittedInfo.size());
      this.getNumber().validate(compatible);
      this.getBlockHash().validate(compatible);
      this.getTransactionHash().validate(compatible);
    }
    static size() {
      return 0 + Uint64.size() + Byte32.size() + Byte32.size();
    }
  }

  function SerializeL2BlockCommittedInfo(value) {
    const array = new Uint8Array(0 + Uint64.size() + Byte32.size() + Byte32.size());
    new DataView(array.buffer);
    array.set(new Uint8Array(SerializeUint64(value.number)), 0);
    array.set(new Uint8Array(SerializeByte32(value.block_hash)), 0 + Uint64.size());
    array.set(new Uint8Array(SerializeByte32(value.transaction_hash)), 0 + Uint64.size() + Byte32.size());
    return array.buffer;
  }
  ...
  class TransactionKey {
    ...
    validate(compatible = false) {
      assertDataLength(this.view.byteLength, 36);
    }
  ...
  function SerializeTransactionKey(value) {
    ...
    assertDataLength(buffer.byteLength, 36);
    return buffer;
  }
  ...
  class TransactionInfo {
    ...
    getKey() {
      return new TransactionKey(this.view.buffer.slice(0 + Uint64.size(), 0 + Uint64.size() + TransactionKey.size()), { validate: false });
    }

    validate(compatible = false) {
      assertDataLength(this.view.byteLength, TransactionInfo.size());
      this.getBlockNumber().validate(compatible);
      this.getKey().validate(compatible);
    }
    static size() {
      return 0 + Uint64.size() + TransactionKey.size();
    }
  }

  function SerializeTransactionInfo(value) {
    const array = new Uint8Array(0 + Uint64.size() + TransactionKey.size());
    new DataView(array.buffer);
    array.set(new Uint8Array(SerializeUint64(value.block_number)), 0);
    array.set(new Uint8Array(SerializeTransactionKey(value.key)), 0 + Uint64.size());
    return array.buffer;
  }
  ...
  class LogItem {
    ...
    validate(compatible = false) {
      ...
      if (offsets[2] - offsets[1] !== 1) {
        throw new Error(`Invalid offset for service_flag: ${offsets[1]} - ${offsets[2]}`)
      }
  ...
  function SerializeLogItem(value) {
    const buffers = [];
    buffers.push(SerializeUint32(value.account_id));
    const serviceFlagView = new DataView(new ArrayBuffer(1));
    serviceFlagView.setUint8(0, value.service_flag);
    buffers.push(serviceFlagView.buffer);
    buffers.push(SerializeBytes(value.data));
  ...
  class LogItemVec {
    ...
    validate(compatible = false) {
      const offsets = verifyAndExtractOffsets(this.view, 0, true);
      for (let i = 0; i < offsets.length - 1; i++) {
        new LogItem(this.view.buffer.slice(offsets[i], offsets[i + 1]), { validate: false }).validate();
      }
    }
    ...
    indexAt(i) {
      ...
      return new LogItem(this.view.buffer.slice(offset, offset_end), { validate: false });
    }
  }

  function SerializeLogItemVec(value) {
    return serializeTable(value.map(item => SerializeLogItem(item)));
  }
  ...
  class TxReceipt {
    ...
    validate(compatible = false) {
      ...
      new AccountMerkleState(this.view.buffer.slice(offsets[1], offsets[2]), { validate: false }).validate();
      new Byte32Vec(this.view.buffer.slice(offsets[2], offsets[3]), { validate: false }).validate();
      new LogItemVec(this.view.buffer.slice(offsets[3], offsets[4]), { validate: false }).validate();
    }
    ...
    getLogs() {
      ...
      const offset = this.view.getUint32(start, true);
      const offset_end = this.view.byteLength;
      return new LogItemVec(this.view.buffer.slice(offset, offset_end), { validate: false });
    }
  }

  function SerializeTxReceipt(value) {
    const buffers = [];
    buffers.push(SerializeByte32(value.tx_witness_hash));
    buffers.push(SerializeAccountMerkleState(value.post_state));
    buffers.push(SerializeByte32Vec(value.read_data_hashes));
    buffers.push(SerializeLogItemVec(value.logs));
    return serializeTable(buffers);
  }
  ...
  function SerializeWithdrawalReceipt(value) {
    const buffers = [];
    buffers.push(SerializeAccountMerkleState(value.post_state));
    return serializeTable(buffers);
  }
  ...
  class SMTBranchKey {
    ...
    validate(compatible = false) {
      assertDataLength(this.view.byteLength, SMTBranchKey.size());
      this.getNodeKey().validate(compatible);
    }
    static size() {
  ...
  function SerializeSMTBranchKey(value) {
    const array = new Uint8Array(0 + 1 + Byte32.size());
    const view = new DataView(array.buffer);
    view.setUint8(0, value.height);
    array.set(new Uint8Array(SerializeByte32(value.node_key)), 0 + 1);
    return array.buffer;
  }
  ...
  class SMTBranchNode {
    ...
    validate(compatible = false) {
      const offsets = verifyAndExtractOffsets(this.view, 0, true);
      new SMTMergeValue(this.view.buffer.slice(offsets[0], offsets[1]), { validate: false }).validate();
      new SMTMergeValue(this.view.buffer.slice(offsets[1], offsets[2]), { validate: false }).validate();
    }
    ...
    getLeft() {
      ...
      const offset = this.view.getUint32(start, true);
      const offset_end = this.view.getUint32(start + 4, true);
      return new SMTMergeValue(this.view.buffer.slice(offset, offset_end), { validate: false });
    }
  ...
  function SerializeSMTBranchNode(value) {
    const buffers = [];
    buffers.push(SerializeSMTMergeValue(value.left));
    buffers.push(SerializeSMTMergeValue(value.right));
    return serializeTable(buffers);
  }
  ...
  class SMTMergeValue {
    ...
    validate(compatible = false) {
      ...
      switch (t) {
      case 0:
        new SMTValue(this.view.buffer.slice(4), { validate: false }).validate();
        break;
      case 1:
        new SMTMergeWithZero(this.view.buffer.slice(4), { validate: false }).validate();
        break;
      default:
    ...
    value() {
      ...
      switch (t) {
      case 0:
        return new SMTValue(this.view.buffer.slice(4), { validate: false });
      case 1:
        return new SMTMergeWithZero(this.view.buffer.slice(4), { validate: false });
      default:
        throw new Error(`Invalid type: ${t}`);
      }
    }
  ...
  function SerializeSMTMergeValue(value) {
    switch (value.type) {
    case "SMTValue":
      {
        const itemBuffer = SerializeSMTValue(value.value);
    ...
    case "SMTMergeWithZero":
      {
        const itemBuffer = SerializeSMTMergeWithZero(value.value);
        const array = new Uint8Array(4 + itemBuffer.byteLength);
        const view = new DataView(array.buffer);
  ...
  class SMTMergeWithZero {
    ...
    getZeroCount() {
      return this.view.getUint8(0 + Byte32.size() + Byte32.size());
    }

    validate(compatible = false) {
      assertDataLength(this.view.byteLength, SMTMergeWithZero.size());
      this.getBaseNode().validate(compatible);
      this.getZeroBits().validate(compatible);
    }
    static size() {
      return 0 + Byte32.size() + Byte32.size() + 1;
    }
  }

  function SerializeSMTMergeWithZero(value) {
    const array = new Uint8Array(0 + Byte32.size() + Byte32.size() + 1);
    const view = new DataView(array.buffer);
    array.set(new Uint8Array(SerializeByte32(value.base_node)), 0);
    array.set(new Uint8Array(SerializeByte32(value.zero_bits)), 0 + Byte32.size());
    view.setUint8(0 + Byte32.size() + Byte32.size(), value.zero_count);
    return array.buffer;
  }
  ...
  class SMTValue {
    ...
    validate(compatible = false) {
      assertDataLength(this.view.byteLength, SMTValue.size());
      this.getValue().validate(compatible);
    }
    static size() {
  ...
  function SerializeSMTValue(value) {
    ...
    new DataView(array.buffer);
    array.set(new Uint8Array(SerializeByte32(value.value)), 0);
    return array.buffer;
  }
  ...
  exports.SMTBranchKey = SMTBranchKey;
  exports.SMTBranchNode = SMTBranchNode;
  exports.SMTMergeValue = SMTMergeValue;
  exports.SMTMergeWithZero = SMTMergeWithZero;
  ...
  exports.SerializeL2BlockCommittedInfo = SerializeL2BlockCommittedInfo;
  exports.SerializeL2Transaction = SerializeL2Transaction;
  exports.SerializeL2TransactionVec = SerializeL2TransactionVec;
  exports.SerializeLogItem = SerializeLogItem;
  exports.SerializeLogItemVec = SerializeLogItemVec;
  exports.SerializeMetaContractArgs = SerializeMetaContractArgs;
  exports.SerializeNumberHash = SerializeNumberHash;
  ...
  exports.SerializeSMTBranchKey = SerializeSMTBranchKey;
  exports.SerializeSMTBranchNode = SerializeSMTBranchNode;
  exports.SerializeSMTMergeValue = SerializeSMTMergeValue;
  exports.SerializeSMTMergeWithZero = SerializeSMTMergeWithZero;
  exports.SerializeSMTValue = SerializeSMTValue;
  ...
  exports.SerializeWithdrawalReceipt = SerializeWithdrawalReceipt;
  ...
  exports.WithdrawalReceipt = WithdrawalReceipt;

...
let _erc20ProxyAddress: HexString;
...
export class main extends Service {
  ...
  async build_deploy() {
    ...
    _erc20ProxyAddress = contractAddress;
    const account_script_hash =
      await this.api.godwokenWeb3Rpc.gw_get_script_hash_by_short_address(
        contractAddress
      );
    const account_id =
      await this.api.godwokenWeb3Rpc.gw_get_account_id_by_script_hash(
        account_script_hash
      );
    return {

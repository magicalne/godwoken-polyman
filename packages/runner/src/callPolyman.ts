  ...
  const run_prepare_money = async (maxRetryLimit: number, intervals = 5000) => {
    try {
      const isMoneyReady = await api.checkCkbBalance(
        user_ckb_devnet_addr,
        user_account_init_amount
      );
      if (isMoneyReady) {
  ...
  const run_prepare_sudt_scripts = async (
    ...
    try {
      ...
      if (isSudtScriptAlreadyExits) {
        ...
        res.send({
          status: "ok",
          data: "sudt script already prepared. finished~",
        });
      } else {
        retry++;
      ...
      if (retry < maxRetryLimit) {
        ...
        res.send({
          status: "failed",
          data: "failed to prepare sudt script for lumos.",
        });
      }
    }
  };
...
app.get("/gen_config", async function (req, res) {
  ...
  try {
    ...
    res.send({
      status: "ok",
      data: "generated new godwoken config for polyman.",
    });
  } catch (error) {
  ...
  try {
    ...
    const run_deploy_script = async (
      ...
      try {
        ...
        const contract_code_hash = await readScriptCodeHashFromFile(
          script_path
        );
    ...
    if (deploy_counter === 0 && total_scripts_number === 0) {
      const result = {
        status: "ok",
        data: `all godwoken scripts are already deployed. skip~`,
      };
      console.log(result);
    ...
    if (deploy_counter === total_scripts_number) {
      ...
      const result = {
        status: "ok",
        data: `${total_scripts_number} scripts deployed. generate deploy Result File: ${isDeployResultFileGenerated}. finished~`,
      };
      console.log(result);
      return res.send(result);
    }
    ...
    const result = {
      status: "failed",
      data: `only able to deployed ${deploy_counter} scripts, required total: ${total_scripts_number}. try again to deploy the rest.`,
    };
    console.log(result);
    res.send(result);
  } catch (error) {
  ...
  try {
    ...
    const run_deploy_script = async (
      ...
      try {
        ...
        console.log(
          `deploy script ${script_name}, outpoint: ${JSON.stringify(
            outpoint
        ...
        res.send({
          status: "ok",
          data: `deploy script ${script_name}, outpoint: ${JSON.stringify(
            outpoint
          )}. finished~`,
        });
      } catch (e) {
        ...
        if (retry < maxRetryLimit) {
          ...
          res.send({
            status: "failed",
            data: `failed to deploy godwoken script ${script_name}.`,
          });
        }
      }
    };
...
export function start() {
  app.listen(PolymanConfig.prepare_service_port, () => {
    console.log(
      `CallPolyman Server(setup service) started at http://localhost:${PolymanConfig.prepare_service_port}`
    );
  });
}

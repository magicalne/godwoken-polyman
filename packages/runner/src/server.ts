...
const setUpRouters = (
    ...
    try {
      ...
      switch (type) {
        case "create_creator":
          ...
          res.send({
            status: "ok",
            data: { run_result: tx_hash, account_id: id },
          });
          break;

        case "deploy":
          const contract_id = await api.waitForDeolyedContractOnChain(
            raw_l2tx,
            rollup_type_hash
          );
          ...
          const contract_address = await api.polyjuice!.accountIdToAddress(
            contract_id
          );
          res.send({
            status: "ok",
  ...
  app.get("/deposit", async (req, res) => {
    try {
      ...
      res.send({
        status: "ok",
        data: { eth_address: eth_address, account_id: account_id },
      });
    } catch (error) {
      console.log(error);
      res.send({ status: "failed", error: error.message });
  ...
  app.get("/create_creator_account", async (req, res) => {
    try {
      ...
      await api.syncToTip();
      const data = await api.generateCreateCreatorAccountTx(
        from_id,
        sudt_id_str,
        rollup_type_hash
      );
  ...
  app.post("/deploy_contract", async (req, res) => {
    try {
      ...
      if (!creator_account_id)
        return res.send({
          status: "failed",
          error: `creator_account_id not found.`,
        });

      const contract_code = req.body.data.contract_code + "";
      const eth_address = req.body.data.eth_address + "";
      await api.syncToTip();
      const data = await api.generateDeployTx(
  ...
  app.post("/deploy_erc20_proxy_contract", async (req, res) => {
    try {
      ...
      if (!sudt_id)
        return res.send({
          status: "failed",
          error: `sudt account not exits. deposit sudt first.`,
        });
  ...
  app.get("/get_layer2_balance", async (req, res) => {
    try {
      ...
      if (!account_id)
        return res.send({
          status: "failed",
          error: `account not exits. deposit first.`,
        });
  ...
  app.get("/get_contract_addr_by_account_id", async (req, res) => {
    try {
      ...
      await api.syncToTip();
      const account_script_hash = await api.getScriptHashByAccountId(
        account_id
      );
      const contract_addr =
        "0x" +
  ...
  app.get("/get_layer2_sudt_balance", async (req, res) => {
    try {
      ...
      const balance = await api.godwoken.getBalance(
        parseInt(sudt_id + ""),
        account_id
      );
      res.send({ status: "ok", data: balance.toString() });
